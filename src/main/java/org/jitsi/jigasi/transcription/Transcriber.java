    ...
    implements ReceiveStreamBufferListener
{
    ...
    /**
     * Map of injected SSRC -> expiry epoch millis. SSRCs present in this map
     * should be ignored by the transcriber until their expiry time.
     */
    private static final java.util.concurrent.ConcurrentHashMap<Long, Long> injectedSsrcExpiries
        = new java.util.concurrent.ConcurrentHashMap<>();

    private static final long DEFAULT_INJECTED_SSRC_TTL_MS = Long.getLong(
        "org.jitsi.jigasi.transcription.injected_ssrc_ttl_ms", 30_000L);

    /**
     * Register an injected SSRC to be ignored by the transcriber for the default TTL.
     */
    public static void addInjectedSsrc(long ssrc)
    {
        addInjectedSsrc(ssrc, DEFAULT_INJECTED_SSRC_TTL_MS);
    }

    /**
     * Register an injected SSRC to be ignored by the transcriber for the given TTL (ms).
     */
    public static void addInjectedSsrc(long ssrc, long ttlMs)
    {
        long key = ssrc & 0xffffffffL;
        long expiry = System.currentTimeMillis() + Math.max(0, ttlMs);
        injectedSsrcExpiries.put(key, expiry);
    }

    /**
     * Remove an injected SSRC from the ignore map.
     */
    public static void removeInjectedSsrc(long ssrc)
    {
        injectedSsrcExpiries.remove(ssrc & 0xffffffffL);
    }
    ...
    public void bufferReceived(ReceiveStream receiveStream, Buffer buffer)
    {
        ...
        // Evict expired injected SSRC entries
        long now = System.currentTimeMillis();
        for (java.util.Map.Entry<Long, Long> e : injectedSsrcExpiries.entrySet())
        {
            if (e.getValue() <= now)
                injectedSsrcExpiries.remove(e.getKey());
        }

        Long expiry = injectedSsrcExpiries.get(ssrc);
        if (expiry != null && expiry > now)
        {
            if (logger.isDebugEnabled())
                logger.debug("Ignoring injected SSRC " + ssrc + " until " + expiry);
            return;
        }
