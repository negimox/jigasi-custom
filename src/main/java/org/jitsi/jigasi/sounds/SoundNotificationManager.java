...
public class SoundNotificationManager
{
    ...
    /**
     * Inject an Opus Ogg stream into the given media stream. Similar to
     * {@link #injectSoundFileInStream} but consumes the provided InputStream
     * instead of loading a resource from the classpath. Caller is responsible
     * for closing the InputStream.
     */
    public static long injectOpusStream(MediaStream stream, java.io.InputStream opusInputStream)
        throws Throwable
    {
        OpusFile of = new OpusFile(new OggPacketReader(opusInputStream));

        OpusAudioData opusAudioData;
        // Random timestamp, ssrc and seq
    int seq = new Random().nextInt(0xFFFF);
    long ts = new Random().nextInt(0xFFFF);
    long ssrc = (long)(new Random().nextInt() & 0xffffffffL);
        byte pt = stream.getDynamicRTPPayloadType(Constants.OPUS);
        long timeForNextPacket = System.currentTimeMillis();
        long sentDuration = 0;
